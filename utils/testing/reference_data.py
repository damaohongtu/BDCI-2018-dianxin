...
This class also provides a method to generate reference input.
...
class BaseTest(tf.test.TestCase):
  """TestCase subclass for performing reference input tests."""
  ...
  def data_root(self):
    ...
    Returns:
      The path prefix for reference input.
    """
  ...
  def _evaluate_test_case(self, name, graph, ops_to_eval, correctness_function):
    """Determine if a graph agrees with the reference input.
  ...
  def _save_or_test_ops(self, name, graph, ops_to_eval=None, test=True,
    ...
    Args:
      ...
      test: Boolean. If True this function will test graph correctness, load
        weights, and compute numerical values. If False the necessary test input
...
class ReferenceDataActionParser(argparse.ArgumentParser):
  ...
  def __init__(self):
    ...
    self.add_argument(
        "--regenerate", "-regen",
        action="store_true",
        help="Enable this flag to regenerate test input. If not set unit tests"
             "will be run."
    )
