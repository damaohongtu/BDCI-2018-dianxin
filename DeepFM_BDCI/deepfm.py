...
class DeepFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            self.y_deep = tf.reshape(self.embeddings,
                                     shape=[-1, self.field_size * self.embedding_size])  # None * (F*K)
            ...
            self.predict_class = tf.argmax(self.out, axis=-1)

            self.loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.out,
                                                                       labels=tf.squeeze(self.label,axis=-1))
            if self.l2_reg > 0:
    ...
    def _initialize_weights(self):
        ...
        glorot = np.sqrt(2.0 / (input_size + self.n_class))
        weights["concat_projection"] = tf.Variable(
            np.random.normal(loc=0, scale=glorot, size=(input_size, self.n_class)),
            dtype=np.float32)  # layers[i-1]*layers[i]
        weights["concat_bias"] = tf.Variable(tf.constant(value=0.01, shape=[self.n_class]), dtype=np.float32)
        return weights
    ...
    def predict(self, Xi, Xv):
        ...
        while len(Xi_batch) > 0:
            ...
            batch_out = self.sess.run(self.predict_class, feed_dict=feed_dict)
    ...
    def evaluate(self, Xi, Xv, y):
        ...
        return self.eval_metric(y, y_pred, average='macro')
