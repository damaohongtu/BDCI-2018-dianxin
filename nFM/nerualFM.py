...
def parse_args():
    ...
    parser.add_argument('--layers', nargs='?', default='[128, 128, 128]',
                        help="Size of each layer.")
    parser.add_argument('--keep_prob', nargs='?', default='[0.9,0.9,0.9,0.9]',
...
class NeuralFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            self.train_labels = tf.placeholder(tf.int32, shape=[None, 1])  # None * 1
            ...
            self.FM = tf.matmul(self.FM, self.weights['prediction'])  # None * 15
            self.out = self.FM + self.weights['prediction_bias'] # None * 15


            self.predicts = tf.argmax(self.out, axis=-1)
            if self.lamda_bilinear > 0:
                self.loss = tf.nn.sparse_softmax_cross_entropy_with_logits(
                    labels=tf.squeeze(self.train_labels, axis=-1),
                    logits=self.out) + tf.contrib.layers.l2_regularizer(
                    self.lamda_bilinear)(self.weights['feature_embeddings'])  # regulizer
            else:
                self.loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=tf.squeeze(self.train_labels,axis=-1),
                                                                           logits=self.out)
    ...
    def _initialize_weights(self):
        ...
        if num_layer > 0:
            ...
            all_weights['prediction'] = tf.Variable(np.random.normal(loc=0, scale=glorot, size=(self.layers[-1], 15)),
                                                    dtype=np.float32)  # layers[-1] * 15
            all_weights['prediction_bias'] = tf.Variable(np.ones([15], dtype=np.float32))
        else:
    ...
    def train(self, Train_data, Validation_data, Test_data):  # fit a dataset
        ...
        if self.verbose > 0:
            t2 = time()
            init_train = self.evaluate(Train_data, 0)
            init_valid = self.evaluate(Validation_data, 0)
            print("Init: \t train=%.4f, validation=%.4f, [%.1f s]" % (
            init_train, init_valid, time() - t2))
        ...
        for epoch in range(self.epoch):
            ...
            # output validation
            train_result = self.evaluate(Train_data, epoch)

            valid_result = self.evaluate(Validation_data, epoch)
            ...
            # if self.verbose > 0 and epoch % self.verbose == 0:
            #     print("Epoch %d [%.1f s]\t validation=%.4f  [%.1f s]"
    ...
    def predict(self, data, class_dict):
        user_id = data['user_id']
        feed_dict = {self.train_features: data['X'],
                     self.dropout_keep: self.no_dropout,
                     self.train_phase: False}
        predictions = self.sess.run((self.predicts), feed_dict=feed_dict)
        predictions = [str(class_dict[pre]) for pre in predictions]
        columns = ['user_id', 'predict']
        result = pd.DataFrame({'user_id': user_id, 'predict': predictions}, columns=columns)
        result.to_csv('result_pre.csv', index=False)
...
if __name__ == '__main__':
    ...
    model.predict(data=data.Test_data, class_dict=data.class_dict_reverse)
